...
exports.deleteById = (req, res, next) => {
  Roster.findById(req.params.id, (err, player) => {
    ...
    return Roster.findOneAndDelete(req.params.id, errFind => {
      if (errFind) return res.status(400).send({ success: false });
      return res.status(200).send({ sucess: true, message: 'Player deleted from roster' });
    });
  });
};
